...
for i in Human-BRCA2-cds.fasta Sylco.fas; do
...
for i in  chr12.fas; do
    echo $TOOL -i $TESTS/$i --stats=$OUTPUTS/$i.stats --bwt=$OUTPUTS/$i.bwt
    $TOOL -i $TESTS/$i --stats=$OUTPUTS/$i.stats --bwt=$OUTPUTS/$i.bwt;
done
